...
const Skills = () => {
  ...
  const fetchData = async () => {
    try {
      const { data } = await axios.get(`${Baseurl}api/v1/course/Skill/all`);
      setData(data.data);
  ...
  const deleteHandler = async (id) => {
    try {
      const { data } = await axios.delete(
        `${Baseurl}api/v1/course/Skill/delete/${id}`,
        Auth
      );
  ...
  function MyVerticallyCenteredModal(props) {
    ...
    const payload = { title, description, descriptionPoints };
    ...
    const postHandler = async (e) => {
      ...
      try {
        const { data } = await axios.post(
          `${Baseurl}api/v1/course/Skill/addSkill`,
          payload,
          Auth
        );
        const msg = data.message;
    ...
    const putHandler = async (e) => {
      ...
      try {
        const { data } = await axios.put(
          `${Baseurl}api/v1/course/Skill/edit/${id}`,
          payload,
          Auth
        );
        const msg = data.message;
